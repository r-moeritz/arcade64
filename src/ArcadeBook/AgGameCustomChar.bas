...
5 VM=0:CC=14:CH=VM+CC*1024:RM=53248-CH:GOSUB 60090:GOSUB 60405
6 print"{clear}":gosub 60040
...
120 l=l-1:if l=1024+800 then poke l+cm,5:poke l+1,32 + 128: l=1024+839
...
60200 FOR I=CH TO CH+2048:POKE I,PEEK(I+RM):NEXT:REM move chars
60300 POKE 1,PEEK(1) OR 4:POKE 56334,PEEK(56334)OR 1:REM Switch in I/O
60400 RETURN
60405 FOR I=CH+8*41 TO CH+8*42 STEP 8:FOR J=I TO I+7:READ N:POKE J,N:NEXT:NEXT
60408 return
60410 rem AgTest
60415 DATA 169,102,189,255,60,126,255,255
60420 DATA 169,102,189,255,60,126,255,255
